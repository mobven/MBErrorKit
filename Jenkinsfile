...
node {
    ...
    def WORKSPACE = pwd();
    // Git Configuration
    def REPO_URL = "https://github.com/mobven/ErrorKit.git";
    // SonarQube Configuration
    def SONAR_PROJECT_KEY = "com.mobven.ios.errorkit.sb";
    def SONAR_PROJECT_NAME = "ErrorKit";
    ...
    stage ('Test') {
        try {
            ...
            sh "xcodebuild -scheme MBErrorKit -sdk iphonesimulator -derivedDataPath Build/ -destination 'platform=iOS Simulator,name=iPhone 11,OS=13.4' test -enableCodeCoverage YES"
        } catch (e) {
            sh "bash ${SLACK_SH} 'ErrorStage' '${ts}' ${SLACK_DATA}"
            sendMail(committerEmail);
            currentBuild.result = 'ABORTED'
